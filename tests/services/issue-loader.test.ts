...
describe('loadIssues — happy path', () => {
	...
	it('loads a single valid issue and exposes sourcePath', async () => {
		...
		await fs.writeTextFile('.quill.md/issues/open/1-only-issue.md', text);

		const loaded = await loadIssues(fs);
		expect(loaded).toHaveLength(1);
		expect(loaded[0]?.issue.id).toBe('1');
		expect(loaded[0]?.issue.fields.title).toBe('Only issue');
		expect(loaded[0]?.sourcePath).toBe('.quill.md/issues/open/1-only-issue.md');
	...
	it('loads a valid file alongside a file with a bad integrity hash (bad file is flagged, not dropped)', async () => {
		...
		await fs.writeTextFile('.quill.md/issues/open/1-good.md', good);
		await fs.writeTextFile('.quill.md/issues/open/2-bad.md', tampered);
	...
	it('ignores non-`.md` files in the issues directory', async () => {
		...
		await fs.writeTextFile('.quill.md/issues/open/1-only-md.md', good);
		// Stray files in the directory are not picked up by the loader.
		await fs.writeTextFile('.quill.md/issues/open/notes.txt', 'ignore me');
		await fs.writeTextFile('.quill.md/issues/open/README', 'ignore me');
		await fs.writeTextFile('.quill.md/issues/open/.DS_Store', 'mac metadata');
