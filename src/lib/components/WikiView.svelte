<script lang="ts">
	...
	async function save() {
		...
		try {
			...
			if (typeof w.writeTextFile === 'function') {
				const doc = await saveDocument(w, '.quill.md/wiki', selectedFile, documentId, isImmutable, fileContent);
				fileContent = doc.content;
				originalContent = doc.content;
				documentId = doc.id;
				isImmutable = doc.immutable;
	...
	async function lockDocument() {
		...
		try {
			const w = adapter as unknown as { writeTextFile: (p: string, c: string) => Promise<void> };
			if (typeof w.writeTextFile === 'function') {
				const doc = await saveDocument(w, '.quill.md/wiki', selectedFile, documentId, true, fileContent);
				fileContent = doc.content;
				originalContent = doc.content;
				documentId = doc.id;
				isImmutable = doc.immutable;
		...
		if (name && name.trim()) {
			...
			try {
				...
				if (typeof w.writeTextFile === 'function') {
					...
					await saveDocument(w, '.quill.md/wiki', filename, id, false, `# ${name}\n\nStart writing here...`);
					await loadFiles();
