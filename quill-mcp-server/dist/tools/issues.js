...
export async function listIssues(projectDir) {
    ...
    try {
        ...
        return {
            content: [{ type: 'text', text: `Error reading issues: ${msg}` }],
            isError: true
        };
    }
}
export async function readIssue(issueId, projectDir) {
    ...
    try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `Error reading issue: ${msg}` }],
            isError: true
        };
    }
}
export async function createIssue(title, issueType, status, sections, relations, customFields, projectDir) {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Strict Validation Failed: Template for issue type '${issueType}' does not exist.`, { cause: e });
        }
        ...
        const issueSections = Object.entries(sections).map(([key, markdown]) => {
            let tplSecName = key;
            if (template.sections && Array.isArray(template.sections)) {
                const tplSec = template.sections.find((s) => s.key === key);
        ...
        const msg = error instanceof Error ? error.message : String(error);
        return {
            content: [{ type: 'text', text: `Error creating issue: ${msg}` }],
            isError: true
        };
    }
}
